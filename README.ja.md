<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=6e40c9&height=120&section=header"/>

<div align="center">

# Victor Gabriel Prado Harger

**システム開発専攻の学生 · バックエンド開発者を目指しています**

[🇧🇷 PT-BR](README.md) · [🇺🇸 English](README.en.md)

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code\&weight=500\&size=22\&pause=1000\&color=6e40c9\&center=true\&vCenter=true\&width=600\&lines=Java+%7C+Spring+Boot+%7C+C%23;Python+%7C+SQL+%7C+REST+APIs;)](https://git.io/typing-sv)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge\&logo=linkedin\&logoColor=white)](https://linkedin.com/in/victor-gabriel-prado-harger)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge\&logo=gmail\&logoColor=white)](mailto:v.gabrielpradoharger@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge\&logo=github\&logoColor=white)](https://github.com/Victor-Harger)

</div>

---

<div align="center">

## 自己紹介

</div>

現在、Centro Universitário Senacでシステム分析・開発（Análise e Desenvolvimento de Sistemas）を専攻しており、バックエンド開発を中心に学んでいます。

これからも、理論的な知識を実際に動くソフトウェアへとつなげるプロジェクトに取り組み、ソフトウェア開発分野での実務経験を積むことを目指しています。

---

<div align="center">

## 主なプロジェクト

</div>

<div align="center">

|                                    プロジェクト                                   |               概要              |                   技術                   |
| :-------------------------------------------------------------------------: | :---------------------------: | :------------------------------------: |
|   [Noctis Hall](https://github.com/Victor-Harger/Site_CasaDeShow_Angular)   |  イベント会場のチケット管理・販売を行うWebシステム。  |   `Angular` `TypeScript` `JavaScript`  |
|       [Projeto ODASU](https://github.com/Victor-Harger/Projeto-Odasu)       |    美容業界向けの商品販売・管理プラットフォーム。    | `Java` `Spring Boot` `SQL Server` `C#` |
| [顔認識システム](https://github.com/Victor-Harger/Reconhecimento_Facial_Em_Python) | 画像処理技術を使用したリアルタイム顔認識アプリケーション。 |        `Python` `OpenCV` `dlib`        |

</div>

---

<div align="center">

## 技術スタック

</div>

<div align="center">

### バックエンド

<p align="center">
  <img src="https://skillicons.dev/icons?i=java,spring,cs,python,nodejs,postman,git&theme=dark" />
</p>

### Web

<p align="center">
  <img src="https://skillicons.dev/icons?i=angular,ts,js,html,css&theme=dark" />
</p>

### データベース

<p align="center">
  <img src="https://skillicons.dev/icons?i=mysql,mongodb&theme=dark" />
  <img src="https://www.svgrepo.com/show/303229/microsoft-sql-server-logo.svg" width="48" height="48" />
  <img src="https://www.vectorlogo.zone/logos/mariadb/mariadb-icon.svg" width="48" height="48" />
</p>

### モバイル・ゲーム開発・その他

<p align="center">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/android/android-original-wordmark.svg" width="48" height="48"/>
  <img src="https://skillicons.dev/icons?i=unity,blender,figma,arduino,swift&theme=dark" />
</p>

---

<div align="center">

## 現在学習中

<p align="center">
  <img src="https://skillicons.dev/icons?i=docker,kubernetes,aws&theme=dark" />
</p>

**Docker · Kubernetes · AWS · Cloud Infrastructure**

</div>

---

<div align="center">

## 学歴・学習歴

|     |                                                               |
| :-: | :------------------------------------------------------------ |
|  🎓 | **システム分析・開発（ADS） — Senac** *(3学期 · 2027年卒業予定)*                |
|  🏫 | **システム開発技術課程 — Etec / Centro Paula Souza** *(2024年修了)*        |
|  🐍 | **CS50P — Introduction to Programming with Python** · Harvard |
|  🎮 | **CS50G — Introduction to Game Development** · Harvard        |
|  ☁️ | **AI-900 — Azure AI Fundamentals** · Microsoft                |

</div>

---

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=6e40c9&height=80&section=footer"/>
